import { Dust, Fire, Explosion } from './particles.js';
const states = {
  SITTING: 0,
  RUNNING: 1,
  JUMPING: 2,
...
export class Diving extends State{
  ...
  handleInput(input) {
    ...
    if (this.game.player.onGround()) {
      this.game.player.setState(states.RUNNING, 1);
      for (let i = 0; i < 30; i++) {
        this.game.particles.unshift(new Explosion(this.game, this.game.player.x + this.game.player.width * 0.3, this.game.player.y + this.game.player.height));
      }
    }else if (input.includes('Enter') && this.game.player.onGround()) {
      this.game.player.setState(states.ROLLING, 2);
    }
